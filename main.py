...
# Removed background processing - back to synchronous
...
# Removed background processing utilities
...
async def agent_think_and_act_endpoint(
    ...
    try:
        ...
        # Execute agent reasoning synchronously (original behavior)
        ...
        # Log activity in background (keeping the call_control_id)
        duration_ms = int((datetime.now() - start_time).total_seconds() * 1000)
        background_tasks.add_task(
            log_activity_background,
...
# Removed background processing function - back to synchronous


# Removed deprecated background result delivery function
...
# Removed debug endpoints for background processing
